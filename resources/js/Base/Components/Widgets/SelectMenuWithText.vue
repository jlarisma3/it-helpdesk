    ...
    <Listbox as="div" v-model="selected">
        ...
        <div class="relative">
            ...
            <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <ListboxOptions class="absolute right-0 z-10 mt-2 w-64 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <ListboxOption as="template" v-for="option in publishingOptions" :key="option.title" :value="option" v-slot="{ active, selected }">
                        <li :class="[active ? 'bg-indigo-600 text-white' : 'text-gray-900', 'px-4 py-2 cursor-default select-none p-4 text-sm']">
...
export default defineComponent({
    ...
    setup() {
        const publishingOptions = [
            { title: 'Open', description: '', current: true },
            { title: 'Queued', description: '', current: false },
            { title: 'Ongoing', description: '', current: false },
            { title: 'Solved', description: '', current: false },
            { title: 'Closed', description: '', current: false },
        ]
